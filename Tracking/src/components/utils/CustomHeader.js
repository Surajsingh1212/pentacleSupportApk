  ...
  return (
    <View className="flex-row w-full p-4 justify-between bg-slate-100">  
      <TouchableOpacity onPress={() => navigation.openDrawer()} className="flex items-center">
       <Text className="text-[#f43439]"> <Icon name="menu" size={32} Text="text-cyan-500" /></Text>
      </TouchableOpacity>
      <View className="flex relative  w-1/2 justify-between">
        <TextInput placeholder="Search..." placeholderTextColor="#f43439"  className="border w-full rounded-full border-[#f43439] px-4 py-1 placeholder:text-[#f43439] text-slate-950 text-md" />
        <TouchableOpacity className="rounded-full px-1 pt-2 absolute top-0 right-0">
          <Text className="text-[#f43439]"><Icon name="magnify" size={24} /></Text>
        </TouchableOpacity>
      </View>
      <TouchableOpacity className="flex items-center" onPress={handleAlertsNavigation}>
      <Text className="text-[#f43439]"><Icon name="bell" size={32} /></Text>
        {notificationCount > 0 && (
